...
[![.NET 版本](https://img.shields.io/badge/.NET-8.0-darkblue?style=flat-square)](https://dotnet.microsoft.com/en-us/download/dotnet/8.0)
...
> [!IMPORTANT]
>
> 当使用“通过发送 ClassIsland Uri”时，确保已经安装并正确配置了 ClassIsland，以便能够接收 SeewoMonitor 发出的 Uri。
...
- rtcRemoteDesktop.exe。
  ...
  >SeewoMonitor 通过监控 `rtcRemoteDesktop.exe` 进程的行为来检测希沃集控的监控行为。当检测到监控行为时，SeewoMonitor 可以通过发送 ClassIsland Uri 或发出系统通知来提醒学生。可能存在延迟或误判的情况。

### 鸣谢

- [TBedu](https://github.com/TBedu)。

  > 本项目一开始由 [TBedu](https://github.com/TBedu) 由 Python 构建，Mistveil-Z 进行重构和维护。
